...
function handleUpload($fileInputName, $existingFile = null) {
    if (isset($_FILES[$fileInputName]) && $_FILES[$fileInputName]['error'] === UPLOAD_ERR_OK) {
        ...
        if (move_uploaded_file($_FILES[$fileInputName]['tmp_name'], $targetPath)) {
            if ($existingFile && file_exists("../uploads/vikasana/" . $existingFile)) {
                @unlink("../uploads/vikasana/" . $existingFile);
            }
            return $fileName;
        }
    }
...
switch ($action) {
    ...
    // Using same logic as Committee for adding designations
    ...
    case 'fetch_single':
        try {
            $id = $_POST['id'];
            $stmt = $pdo->prepare("SELECT * FROM vikasana_members WHERE id = ?");
            $stmt->execute([$id]);
            $data = $stmt->fetch();
            echo json_encode(['status' => 'success', 'data' => $data]);
        } catch (PDOException $e) {
    ...
    case 'update':
        try {
            ...
            if ($action === 'create') {
                $sql = "INSERT INTO vikasana_members (designation_id, name, age, hamlet_id, family_id, date_of_entry, photo) 
                        VALUES (?, ?, ?, ?, ?, ?, ?)";
                $stmt = $pdo->prepare($sql);
                $stmt->execute([$designation_id, $name, $age, $hamlet_id, $family_id, $date_of_entry, $photo]);
                echo json_encode(['status' => 'success', 'message' => 'Member added successfully']);
            } else {
                $id = $_POST['id'];
                $sql = "UPDATE vikasana_members SET designation_id=?, name=?, age=?, hamlet_id=?, family_id=?, date_of_entry=?, photo=? 
                        WHERE id=?";
                $stmt = $pdo->prepare($sql);
                $stmt->execute([$designation_id, $name, $age, $hamlet_id, $family_id, $date_of_entry, $photo, $id]);
    ...
    case 'delete':
        try {
            $id = $_POST['id'];
            $stmt = $pdo->prepare("SELECT photo FROM vikasana_members WHERE id = ?");
            $stmt->execute([$id]);
            $photo = $stmt->fetchColumn();

            if ($photo && file_exists("../uploads/vikasana/" . $photo)) {
                @unlink("../uploads/vikasana/" . $photo);
            }

            $stmt = $pdo->prepare("DELETE FROM vikasana_members WHERE id = ?");
            $stmt->execute([$id]);
